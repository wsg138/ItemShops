...
public final class ShopEditMenu implements ShopMenu {
    ...
    public ShopEditMenu(ItemShopsPlugin plugin, ShopManager mgr, Player owner, Shop shop) {
    ...
    public boolean onClick(InventoryClickEvent e) {
        ...
        if (raw == HOP_IN_SLOT)  { shop.setHopperAllowIn(!shop.isHopperAllowIn());  render(); return true; }
        if (raw == HOP_OUT_SLOT) { shop.setHopperAllowOut(!shop.isHopperAllowOut()); render(); return true; }
        if (raw == SEARCH_SLOT)  { shop.setSearchEnabled(!shop.isSearchEnabled());   render(); return true; }
