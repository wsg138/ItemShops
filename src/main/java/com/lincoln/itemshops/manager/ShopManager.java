package com.lincoln.itemshops.manager;
...
public final class ShopManager {
    ...
    private final Map<Pos, Shop> byContainer = new ConcurrentHashMap<>();

    
    private final Map<Pos, Pos> signToContainer = new ConcurrentHashMap<>();
    ...
    private void drainOnce() {
        ...
        while (processed < batchPerRun) {
            ...
            if (p == null) break;
            queued.remove(p);
            Shop s = byContainer.get(p);
            if (s != null) updateSign(s);
            processed++;
        }
    ...
    public int size(){ return byContainer.size(); }
    public Collection<Shop> all(){ return byContainer.values(); }

    
    public Shop getBySign(Pos signPos){
        Pos cont = signToContainer.get(signPos);
        return cont == null ? null : byContainer.get(cont);
    }
    public Shop byContainer(Pos container){ return byContainer.get(container); }

    
    public List<Shop> shopsOn(Pos uni) {
        Shop s = byContainer.get(uni);
    ...
    public void put(Shop s){
        byContainer.put(s.container(), s);
        
        signToContainer.put(s.sign(), s.container());
        incOwner(s.owner());
    ...
    public void relinkSign(Pos oldSign, Pos newSign, Shop s) {
        if (oldSign != null) signToContainer.remove(oldSign);
        signToContainer.put(newSign, s.container());
        s.setSign(newSign);
    ...
    public void deleteShop(Shop s, RemovalReason reason, UUID actor) {
        if (s == null) return;
        Shop removed = byContainer.remove(s.container());
        if (removed == null) return;
        signToContainer.values().removeIf(v -> v.equals(removed.container()));
        decOwner(removed.owner());
        clearSign(removed);
        storage.saveAsync(this);
        if (plugin.getConfig().getBoolean("debug.removals", true)) {
            plugin.getLogger().info(String.format(Locale.ROOT,
                    "[ItemShops] Removed shop at %s reason=%s actor=%s",
                    removed.container(), reason, actor));
        }
    }
    ...
    public int pruneInvalidShops() {
        int removed = 0;
        for (Shop s : new ArrayList<>(byContainer.values())) {
    ...
    public String canCreateHere(Pos containerPos){
        ...
        if (byContainer.containsKey(containerPos)) {
            return "errors.too-many-on-container"; 
        }
